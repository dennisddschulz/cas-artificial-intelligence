...
class BacktestConfig:
    ...
    start: float = 0.7
    forecast_horizon: int = cfg.model.output_chunk_length # take it from config
    stride: int = 5 # ACHTUNG: hier damit noch spielen
    retrain: bool = False
...
def explain_tft_variables(
    ...
    if encoder_df is not None is not False:
        if isinstance(encoder_df, list):
            for i, df in enumerate(encoder_df):
                print(f"\nEncoder Importance {i}")
                print(df.T)
        else:
            print(encoder_df.T)
            # print(encoder_df.head(50))
