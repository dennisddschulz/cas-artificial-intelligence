#!/usr/bin/env python3
"""
DQN vs Double DQN Training auf LunarLander mit Plots und Visualisierungen
"""
...
def make_env(seed=0, render_mode=None):
    """Erstelle LunarLander Umgebung statt CartPole"""
...
def train_agent(double_dqn=False, total_steps=500_000, seed=0):
    """Trainiere DQN oder Double DQN auf LunarLander"""
    set_seed(seed)
    ...
    eps_start, eps_end, eps_decay = 1.0, 0.01, 200_000
    gamma = 0.99
    ...
    for step in range(1, total_steps + 1):
        ...
        if done:
            ...
            if ep_count % 100 == 0:
                ...
                print(f"Episode {ep_count:4d} | Avg Return (last 100): {avg_return:8.2f} | Eps: {eps:.4f}")
...
print("\n" + "="*60)
print("DQN vs Double DQN auf LunarLander-v3")
print("="*60)

# Train both algorithms
dqn_results = train_agent(double_dqn=False, total_steps=500_000, seed=42)
ddqn_results = train_agent(double_dqn=True, total_steps=500_000, seed=43)
...
ax.set_ylabel("Return (Moving Avg, window=100)", fontsize=14, fontweight="bold")
...
lengths_ma_dqn = moving_average(dqn_results["episode_lengths"], window=100)
lengths_ma_ddqn = moving_average(ddqn_results["episode_lengths"], window=100)
...
ax.set_xlabel("Episode", fontsize=14, fontweight="bold")
ax.set_ylabel("Episode Length (Moving Avg, window=100)", fontsize=14, fontweight="bold")
...
print(f"\nWinner: {'DQN' if mean_dqn > mean_ddqn else 'Double DQN'} (+{abs(mean_dqn - mean_ddqn):.2f} Return)")
print("="*60)
...
summary = f"""
DQN vs Double DQN auf LunarLander-v3
====================================
...
DQN:
  ...
  Mean Return (last 100 eps): {np.mean(dqn_results['episode_returns'][-100:]):.2f}
...
Double DQN:
  ...
  Mean Return (last 100 eps): {np.mean(ddqn_results['episode_returns'][-100:]):.2f}
...
Difference: {abs(mean_dqn - mean_ddqn):.2f} Return Points
"""
