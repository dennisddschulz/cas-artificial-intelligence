...
def _to_single_series_from_hfc(hfc) -> TimeSeries:
    ...
    if isinstance(hfc, list):
        ...
        if isinstance(first, TimeSeries):
            return concatenate(hfc, ignore_time_axis=True)
        ...
        if isinstance(first, list):
            ...
            return concatenate(first, ignore_time_axis=True)
...
def run_darts_backtest(
    ...
    return {
        "historical_forecasts": common_fc,  # Speichern Sie den GEsCHNITTENEN Forecast
        "historical_actual": common_actual,  # <--- FÜGEN SIE DIES HINZU
        "rmse": err_rmse,
        "smape": err_smape,
...
# --------------------------------------------------------------------
# Hilfsfunktion: Log-Renditen in Preise zurückrechnen
# --------------------------------------------------------------------
def reconstruct_price_from_log_return(log_ret: TimeSeries, base_price: float = 100.0) -> TimeSeries:
    """
    Rekonstruiert Preisreihe aus einer log-return-Zeitreihe.
    Der erste Preis wird auf base_price gesetzt.
    """
    import numpy as np
    from darts import TimeSeries

    log_values = log_ret.values().flatten()
    price_values = base_price * np.exp(np.cumsum(log_values))
    return TimeSeries.from_times_and_values(log_ret.time_index, price_values.reshape(-1, 1), columns=["price"])
