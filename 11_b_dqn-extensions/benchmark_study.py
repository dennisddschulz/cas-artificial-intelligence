...
# Shorter training for faster benchmarking
TOTAL_STEPS = 100_000
EVAL_EVERY = 5_000
EVAL_EPISODES = 5
    ...
    def train(self, env_id, total_steps=TOTAL_STEPS, eval_every=EVAL_EVERY, eval_episodes=EVAL_EPISODES):
        ...
        for step in range(1, total_steps + 1):
            ...
            if step % eval_every == 0:
                ...
                print(f"[{self.config['name']:20s} | step {step:7d}] return: {mean_r:7.1f} ± {std_r:5.1f}")
...
if __name__ == "__main__":
    ...
    configs = [
        {'name': 'Vanilla DQN', 'use_double': False, 'use_dueling': False, 'use_noisy': False, 'use_per': False, 'lr': LR},
        {'name': 'Double DQN', 'use_double': True, 'use_dueling': False, 'use_noisy': False, 'use_per': False, 'lr': LR},
        {'name': 'Dueling DQN', 'use_double': False, 'use_dueling': True, 'use_noisy': False, 'use_per': False, 'lr': LR},
        {'name': 'Double + Dueling', 'use_double': True, 'use_dueling': True, 'use_noisy': False, 'use_per': False, 'lr': LR},
        {'name': 'Noisy DQN', 'use_double': False, 'use_dueling': False, 'use_noisy': True, 'use_per': False, 'lr': LR},
        {'name': 'PER (Vanilla)', 'use_double': False, 'use_dueling': False, 'use_noisy': False, 'use_per': True, 'lr': LR},
        {'name': 'Double + Dueling + PER', 'use_double': True, 'use_dueling': True, 'use_noisy': False, 'use_per': True, 'lr': LR},
        {'name': 'All Extensions', 'use_double': True, 'use_dueling': True, 'use_noisy': True, 'use_per': True, 'lr': LR},
    ]
    ...
    print(f"Training steps: {TOTAL_STEPS}, Eval every: {EVAL_EVERY}\n")
    ...
    for name, agent in agents.items():
        ...
        stability = np.mean(agent.eval_stds[-3:])
    ...
    print("\n✓ Saved: benchmark_learning_curves.png")
    plt.close()

    # Visualization 2: Final Performance
    fig, axes = plt.subplots(1, 2, figsize=(16, 6))

    variants = [agent['Variant'] for agent in metrics_summary]
    final_returns = [float(agent['Final Return']) for agent in metrics_summary]
    final_stds = [float(agent['Final Std']) for agent in metrics_summary]
    ...
    ax.set_xlabel('DQN Variant', fontsize=12)
    ax.set_ylabel('Evaluation Std Dev (Lower = More Stable)', fontsize=12)
    ax.set_title('Stability Comparison (Final Std Dev)', fontsize=14, fontweight='bold')
    ax.set_xticks(x_pos)
    ax.set_xticklabels(variants, rotation=45, ha='right')
    ax.grid(True, axis='y', alpha=0.3)
    ...
    # Visualization 3: Learning Speed
    ...
    for name, agent in agents.items():
        ...
        learning_speed_data.append({'Variant': name, 'Steps to 50%': steps_50, 'Steps to 80%': steps_80})

    speed_df = pd.DataFrame(learning_speed_data)
    x_pos = np.arange(len(speed_df))
    width = 0.35

    bars1 = ax.bar(x_pos - width/2, speed_df['Steps to 50%'], width, label='Steps to 50% Performance', alpha=0.8, color='steelblue', edgecolor='black', linewidth=1.5)
    bars2 = ax.bar(x_pos + width/2, speed_df['Steps to 80%'], width, label='Steps to 80% Performance', alpha=0.8, color='coral', edgecolor='black', linewidth=1.5)
    ...
    print("="*100)
    print("\nGenerated files:")
    print("  1. benchmark_learning_curves.png")
    print("  2. benchmark_final_performance.png")
    print("  3. benchmark_learning_speed.png")
    print("  4. benchmark_metrics.csv")
    print("  5. benchmark_detailed_results.csv")
